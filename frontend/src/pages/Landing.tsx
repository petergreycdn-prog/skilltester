...
const features = [
  { icon: '⚡', title: 'API Hono', desc: 'Backend TypeScript no Edge com Cloudflare Workers.' },
  { icon: '🔐', title: 'Auth JWT', desc: 'Registro, login e proteção de rotas com tokens seguros.' },
  { icon: '📧', title: 'E-mails Resend', desc: 'Templates transacionais enviados via Resend API.' },
  { icon: '📊', title: 'Dashboard', desc: 'Gráficos de usuários, e-mails e formulários em tempo real.' },
  { icon: '🗄️', title: 'Cloudflare D1', desc: 'SQLite serverless com Drizzle ORM e migrations.' },
  { icon: '🌙', title: 'Dark Mode', desc: 'UI refinada com Tailwind e animações sutis.' },
]
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative text-center px-6 pt-24 pb-20 overflow-hidden">
        ...
        <div className="inline-block text-xs tracking-widest px-4 py-1.5 rounded-full mb-8 border border-[--border] text-brand-purple bg-brand-purple/10 animate-fade-in">
          ✦ full-stack · TypeScript · Edge
        </div>
        ...
        <p className="text-[#8889aa] text-lg max-w-xl mx-auto mb-10 font-light animate-fade-in-up [animation-delay:0.1s]">
          Aplicação full-stack com autenticação, e-mails transacionais, dashboard com gráficos e banco de dados serverless.
        </p>
      ...
      <section className="max-w-5xl mx-auto px-6 pb-20">
        <p className="text-xs tracking-widest text-brand-blue mb-3 text-center">RECURSOS</p>
        <h2 className="font-display font-bold text-3xl text-center mb-10">Tudo incluso, pronto para produção</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {features.map(f => (
            <div key={f.title} className="card hover:-translate-y-1">
              <div className="text-2xl mb-3">{f.icon}</div>
              <h3 className="font-display font-bold text-base mb-2">{f.title}</h3>
              <p className="text-sm text-[#8889aa] leading-relaxed">{f.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <footer className="text-center py-8 text-xs text-[#5a5c7a] border-t border-[--border]">
        SkillTester · 2026 · Hono + React + Cloudflare
      </footer>
    </div>
  )
}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input className="input" placeholder="Seu nome" required value={form.nome} onChange={e => setForm(p => ({...p, nome: e.target.value}))} />
            </div>
            <div>
              <label className="label">E-mail</label>
              <input type="email" className="input" placeholder="voce@email.com" required value={form.email} onChange={e => setForm(p => ({...p, email: e.target.value}))} />
            </div>
          </div>
          <div>
            <label className="label">Mensagem</label>
            <textarea rows={4} className="input resize-none" placeholder="Como podemos ajudar?" required value={form.mensagem} onChange={e => setForm(p => ({...p, mensagem: e.target.value}))} />
          </div>
          <button type="submit" disabled={status === 'loading'} className="btn-primary justify-center py-3">
